...
export default function Project() {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/project/${id}/comments`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      await axios.post(`${import.meta.env.VITE_API_URL}/project/${id}/comment`, {
        comment: newComment,
        rate: rating
      }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
  ...
  return (
    <div className="container mt-5 project">
      <div className="row">
        {/* Existing image gallery and project details code */}
        ...
        <div className="col-md-5">
          <div className="event-details">
            ...
            <div className="owner mb-3">
              <p>
                Owned by:{' '}
                <span className="cursor-pointer" onClick={handleOwnerClick}>
                ...
                <div className="rating-input mb-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <span
                      ...
                      style={{ cursor: 'pointer', color: star <= rating ? '#4CAF4F' : '#ccc' }}
                    >
                      ★
                    </span>
                  ))}
                    ...
                    <div className="d-flex justify-content-between">
                      ...
                      <div className="rating">
                        {[...Array(comment.rate)].map((_, i) => (
                          <span key={i} style={{ color: '#4CAF4F' }}>★</span>
                        ))}
                      </div>
                    </div>
